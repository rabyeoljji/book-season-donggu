...
- **상세 모달**: 메인 페이지에만 Dialog를 두고 `?place={id}` 쿼리로 제어합니다. 마커 오버레이의 "자세히 보기"가 쿼리를 세팅하고, 유효한 장소(비활성화 제외)일 때만 모달이 열립니다.
...
- 권장 수동 테스트
  - 지도 로딩 및 마커 클릭
  - 상세 모달 진입 및 공유 가능한 라우트 확인 (`/?place={id}`)
...
- 장소 상세 모달은 `usePlaceSelection` 훅을 통해 쿼리 파라미터를 관리하며, 숫자가 아닌 값이나 비활성화된 id는 알림 후 쿼리를 제거합니다.
